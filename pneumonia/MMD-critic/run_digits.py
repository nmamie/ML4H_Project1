...
def plotfigs2(xx, selectedy, fileprefix=None, printselectionnumbers = False):
    ...
    while counter < int(totm/perpic_m) + 1:
        ...
        for i in np.array(range(begin_at + offset, endd)):
            # if selectedy is 0 then it is normal otherwise it is pneumonia
            label = None
            if selectedy[i - offset] == 0:
                label = "Normal"
            else:
                label = "Pneumonia"
            ax = fig.add_subplot(gs[i - begin_at])
            #ax.imshow(xx[i - offset, :].reshape((16, 16)), cmap="Greys_r")
            ax.imshow(xx[i - offset, :].reshape((150, 150)), cmap = "Greys_r")
...
def test_1NN(digitsdat, selected, all_test_m):
    ...
    for testm in all_test_m:
        ...
        error, predy, testy = classifier.classify(digitsdat.testX, digitsdat.testy)
        ...
        # plt.savefig(os.path.join(output_prefix, 'images/confusion_matrix' % testm), dpi=2000)
        # plt.close()
        ...
        # # save ROC curve to output folder
        # plt.savefig(os.path.join(output_prefix, 'images/roc_curve' % testm), dpi=2000)
        # plt.close()
...
def main(
    ...
    selected, critselected, digitsdat = run(
            ...
            os.path.join(data_prefix, 'pneumonia_train'),
            gamma,
            m,
            k,
            kerneltype,
            outfig,
            critoutfig,
            os.path.join(data_prefix, 'pneumonia_test.t'))
...
if __name__ == '__main__':
    ...
    do_output_pics = 1
    kernel_type = 0 # 1 for local, 0 for global
